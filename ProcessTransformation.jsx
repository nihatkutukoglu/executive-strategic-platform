...
import { X, Layers, Box, Cpu, FileOutput, Users } from 'lucide-react';
...
const sipocItems = [
  ...
  { id: 'p', letter: 'P', title: 'Process (Süreç)', icon: Cpu, color: 'text-yellow-400', bg: 'bg-yellow-500/10', details: ['Hasat ve Fabrika Kabul', 'Yıkama, Seçme ve Sıkma İşlemleri', 'Pastörizasyon ve Kalite Kontrol', 'Paketleme ve Soğuk Depolama', 'Şikayet Değerlendirme (Roots/Surway)', 'Sevkiyat ve Müşteri Bilgilendirme'] },
          ...
          <div className="bg-slate-900/90 border border-slate-700 rounded-3xl p-8 max-w-lg w-full relative z-10 shadow-[0_0_50px_rgba(0,0,0,0.5)] transform animate-[fadeIn_0.3s_ease-out] border-t-4" style={{ borderTopColor: 'rgba(255,255,255,0.2)' }}>
            ...
            <ul className="space-y-4">
              {activeSipoc.details.map((detail, idx) => (
                <li key={idx} className="flex items-center gap-4 p-4 rounded-xl bg-slate-800/50 border border-slate-700/50 hover:bg-slate-800 transition-colors">
                  <div className={`w-2 h-2 rounded-full ${activeSipoc.color.replace('text', 'bg')}`}></div>
